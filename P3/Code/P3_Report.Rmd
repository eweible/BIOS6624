...
dat_male_lab <- dat_lab %>% filter(SEX == "Male")
dat_female_lab <- dat_lab %>% filter(SEX == "Female")
  # Separate by sex

surv_object_m <- Surv(time = dat_male_lab$time_10, event = dat_male_lab$stroke_event)
surv_object_f <- Surv(time = dat_female_lab$time_10, event = dat_female_lab$stroke_event)
  # Survival objects by sex

## By diabetes
fit_diab_m <- survfit(surv_object_m ~ Diabetes, data = dat_male_lab)
  # Survival fit
p2_m <- ggsurvplot(fit_diab_m, data = dat_male_lab, ylim = c(0.90, 1.00),
           xlab = "", ylab = "Survival (Males)")$plot + theme(legend.position = "bottom", legend.direction = "vertical")
  # Plot
survdiff_diab_m <- survdiff(surv_object_m ~ Diabetes, data = dat_male_lab)
  # Test
fit_diab_f <- survfit(surv_object_f ~ Diabetes, data = dat_female_lab)
p2_f <- ggsurvplot(fit_diab_f, data = dat_female_lab, ylim = c(0.90, 1.00),
           xlab = "", ylab = "Survival (Females)")$plot + 
  theme(legend.position = "none", axis.text.x = element_blank(), axis.line.x = element_blank())
survdiff_diab_f <- survdiff(surv_object_f ~ Diabetes, data = dat_female_lab)

## By high BP
fit_bp_m <- survfit(Surv(time_10, stroke_event) ~ `High_BP`, data = dat_male_lab)
p3_m <- ggsurvplot(fit_bp_m, data = dat_male_lab, ylim = c(0.90, 1.00),
           xlab = "Years", ylab = "")$plot + theme(legend.position = "bottom", legend.direction = "vertical")
survdiff_bp_m <- survdiff(Surv(time_10, stroke_event) ~ `High_BP`, data = dat_male_lab)
fit_bp_f <- survfit(Surv(time_10, stroke_event) ~ `High_BP`, data = dat_female_lab)
p3_f <- ggsurvplot(fit_bp_f, data = dat_female_lab, ylim = c(0.90, 1.00),
           xlab = "", ylab = "")$plot + 
  theme(legend.position = "none", axis.text.x = element_blank(), axis.line.x = element_blank())
survdiff_bp_f <- survdiff(Surv(time_10, stroke_event) ~ `High_BP`, data = dat_female_lab)

## By smoking
fit_smoke_m <- survfit(surv_object_m ~ `Current_Smoker`, data = dat_male_lab)
p4_m <- ggsurvplot(fit_smoke_m, data = dat_male_lab, ylim = c(0.90, 1.00),
           xlab = "", ylab = "")$plot + theme(legend.position = "bottom", legend.direction = "vertical")
survdiff_smoke_m <- survdiff(surv_object_m ~ `Current_Smoker`, data = dat_male_lab)
fit_smoke_f <- survfit(surv_object_f ~ `Current_Smoker`, data = dat_female_lab)
p4_f <- ggsurvplot(fit_smoke_f, data = dat_female_lab, ylim = c(0.90, 1.00),
           xlab = "", ylab = "")$plot + 
  theme(legend.position = "none", axis.text.x  = element_blank(), axis.line.x = element_blank())
survdiff_smoke_f <- survdiff(surv_object_f ~ `Current_Smoker`, data = dat_female_lab)
...
summary(cox_male <- coxph(Surv(time_10, stroke_event) ~ Age + `Systolic BP` + Diabetes + `Current Smoker`,
    data = dat_male_lab))
  # Summarize Cox model
...
summary(cox_female <- coxph(Surv(time_10, stroke_event) ~ Age + `Systolic BP` + Diabetes + `Current Smoker`,
    data = dat_female_lab))
  # Summarize Cox model
...
p1_m <- fig2_male[["1"]] + labs(y = "Age") + theme(axis.title.y = element_text(face = "bold"))
p2_m <- fig2_male[["2"]] + labs(y = "Systolic BP") + theme(axis.title.y = element_text(face = "bold"))
p3_m <- fig2_male[["3"]] + labs(y = "Diabetes") + theme(axis.title.y = element_text(face = "bold"))
p4_m <- fig2_male[["4"]] + labs(y = "Current Smoker") + theme(axis.title.y = element_text(face = "bold"))
  # Separate male figures with labels
p1_f <- fig2_female[["1"]] + theme(axis.title.y  = element_blank())
p2_f <- fig2_female[["2"]] + theme(axis.title.y  = element_blank())
p3_f <- fig2_female[["3"]] + theme(axis.title.y  = element_blank())
p4_f <- fig2_female[["4"]] + theme(axis.title.y  = element_blank())
  # Separate female figures
row1 <- plot_grid(p1_m, p1_f, ncol = 2)
row2 <- plot_grid(p2_m, p2_f, ncol = 2)
row3 <- plot_grid(p3_m, p3_f, ncol = 2)
row4 <- plot_grid(p4_m, p4_f, ncol = 2)
main_grid <- plot_grid(row1, row2, row3, row4, ncol = 1,
                       label_x = 0, hjust = 0)
  # COmbine figures
header <- plot_grid(
  ggdraw() + draw_label("Male", fontface = "bold"),
  ggdraw() + draw_label("Female", fontface = "bold"),
  ncol = 2
)
fig2 <- plot_grid(
  header,
...
stroke_prob_10_male <- pred_male$surv
  # 10-year survival probability = S(10)
...
combined_table <- bind_rows(female_table_final, male_table_final) %>%
  ...
  tab_footnote(
    footnote = "Table 2: Predicted survival probabilities were generated from sex-specific Cox proportional hazards models using baseline age, systolic BP, diabetes, and smoking status. Five predefined clinical profiles were assessed for three ages. High systolic BP is defined as 160 mmHg." 
  ) # Combine tables
...
table1(~ `Age` + `Systolic BP` + Diabetes + `Prior CHD` + `On BP Meds` +
         `Current Smoker` + `Total Cholesterol` + `Body Mass Index`
       | SEX, data = dat_lab, caption = "Table 1: Baseline demographic and clinical characteristics by sex (analytical dataset)")
```

![](Figures/Table1.png)

![Kaplan-Meier survival curves compare 10 year stroke-fre survival across categories of diabetes, high systolic BP (at least 160 mmHg), and current smoking, stratified by sex. Differences between groups were evaluated using log-rank tests. These plots illustrate unadjusted associations between baseline risk factors and stroke incidence.](Figures/Fig1.png)

![Panels display Schoenfeld residuals over time for age, systolic BP, diabetes, and current smoking, stratified by sex. Each plot includes pointwise residuals, a smoothed trend line, and 95% confidence bands. Individual test p-values are reported for each model. Global p-values are 0.2277 for males and 0.9575 for females. These diagnostics support the proportional hazards assumption of the Cox models used to estimate stroke risk.](Figures/Fig2.png)

![](Figures/Tab2.png)

![This heatmap visualizes 10 year stroke-free survival probabilites across combinations of systolic BP, diabetes status, age, and sex. Predictions were obtained from sex-specific Cox models. Color gradients represent survival probability, with darker red indicating higher predicted survival.](Figures/Fig3.png)

![Mean systolic BP and diabetes prevalence were summarized across three Framingham exam periods to evaluate changes in risk factors over time. ](Figures/Fig4.png)
